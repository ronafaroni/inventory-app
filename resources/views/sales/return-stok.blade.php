...
<script>
    ...
    $('#addData').on('click', function() {
        ...
        const tambahStok = $('#return_stok').val();
        
        if ( kodeSales && namaSales && kodeItem && namaItem && returnStok ) {
            const dataObject = { kodeSales, namaSales, kodeItem, namaItem, returnStok };
    ...
    function updateTable() {
        ...
        dataArray.forEach((data, index) => {
            totalStok += parseInt(data.returnStok, 10); // Menambahkan stok item ke total stok
            tableBody.append(`
                <tr>
                    <td>${index + 1}</td>
                    <td>${data.kodeItem}</td>
